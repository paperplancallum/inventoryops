// Generate Replenishment Suggestions Edge Function
// Analyzes inventory levels, forecasts, and safety stock rules to generate
// transfer and purchase order suggestions with urgency classification
...
function calculateDaysOfStock(
  ...
  if (dailyRate <= 0) return 999 // Infinite if no sales
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!amazonLocations || amazonLocations.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: 'No Amazon locations found', suggestions: 0 }),
    ...
    if (!products || products.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: 'No active products found', suggestions: 0 }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    // Fetch suppliers for PO suggestions
    ...
    // Fetch route legs for transit time calculation
    let routeTransitDays = 14 // Default
    let routeMethod = 'sea'
    ...
    safetyRules?.forEach((rule) => {
      ...
      safetyMap.set(key, {
        ...
        seasonalMultipliers: rule.seasonal_multipliers || [],
      })
    })

    // Build warehouse inventory map for source finding
    ...
    for (const product of products) {
      for (const location of amazonLocations) {
        ...
        reasoning.push({
          type: 'calculation',
          message: `Current stock: ${currentStock.toLocaleString()} units`,
          value: currentStock,
        })
        ...
        if (inTransit > 0) {
          reasoning.push({
            type: 'info',
            message: `In transit: ${inTransit.toLocaleString()} units`,
            value: inTransit,
          })
        }
        ...
        reasoning.push({
          type: 'calculation',
          message: `Daily sales rate: ${adjustedDailyRate.toFixed(1)} units/day`,
          value: adjustedDailyRate,
        })

        reasoning.push({
          type: 'calculation',
          message: `Days of stock remaining: ${daysOfStock === 999 ? 'Unlimited' : daysOfStock}`,
          value: daysOfStock,
        })
        ...
        if (daysOfStock <= thresholds.criticalDays) {
          reasoning.push({
            type: 'warning',
            message: `CRITICAL: Stock will run out in ${daysOfStock} days`,
          })
        } else if (daysOfStock <= thresholds.warningDays) {
          reasoning.push({
            type: 'warning',
            message: `WARNING: Stock below ${thresholds.warningDays}-day threshold`,
          })
        }

        reasoning.push({
          type: 'calculation',
          message: `Safety stock threshold: ${safetyThreshold.toLocaleString()} units`,
          value: safetyThreshold,
        })

        reasoning.push({
          type: 'info',
          message: `Recommended replenishment: ${recommendedQty.toLocaleString()} units`,
        ...
        if (bestSource && bestSource.availableQty >= recommendedQty) {
          ...
          transitDays = 7 // Assume 7 days for domestic transfer

          reasoning.push({
            type: 'info',
            message: `Transfer from ${bestSource.locationName} (${bestSource.availableQty.toLocaleString()} available)`,
          })
        } else {
          // PO suggestion
          suggestionType = 'purchase-order'

          // Find preferred supplier (could be enhanced with product-supplier mapping)
          ...
          if (preferredSupplier) {
            ...
            reasoning.push({
              type: 'info',
              message: `Purchase from ${preferredSupplier.name} (${supplierLeadTimeDays} day lead time)`,
            })
          }
          ...
          if (bestSource) {
            reasoning.push({
              type: 'info',
              message: `Warehouse stock insufficient: ${bestSource.availableQty.toLocaleString()} available`,
            })
          } else {
            reasoning.push({
              type: 'info',
              message: 'No warehouse stock available - purchase order required',
            })
          }
        }

        const stockoutDate = calculateStockoutDate(daysOfStock)
        const estimatedArrival = calculateEstimatedArrival(transitDays)
    ...
    console.error('Error in generate-replenishment-suggestions:', error)
    return new Response(
