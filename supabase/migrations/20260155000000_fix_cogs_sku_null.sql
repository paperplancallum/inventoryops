-- Fix: Use p.sku from products table instead of ps.sku from LEFT JOIN
-- The products table has sku as NOT NULL, but product_skus via LEFT JOIN can be NULL
...
BEGIN
  ...
  SELECT
    p.id AS product_id,
    p.sku AS sku,  -- Use p.sku directly (NOT NULL) instead of ps.sku (nullable from LEFT JOIN)
