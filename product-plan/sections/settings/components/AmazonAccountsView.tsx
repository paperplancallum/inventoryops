...
import type { AmazonAccountsViewProps, AmazonConnection, AmazonRegion } from '@/../product/sections/settings/types'
import { AMAZON_MARKETPLACES } from '@/../product/sections/settings/types'

const regionLabels: Record<AmazonRegion, string> = {
  NA: 'North America',
  EU: 'Europe',
  FE: 'Far East',
}

const regionColors: Record<AmazonRegion, string> = {
  NA: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
  EU: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
  FE: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
}
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${regionColors[connection.region]}`}>
              {regionLabels[connection.region]}
            </span>
          </div>

          {/* Seller info */}
          <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400 mb-3">
            <span className="truncate">{connection.sellerName || 'Unknown Seller'}</span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {connection.enabledMarketplaces.map((mp) => {
              const info = AMAZON_MARKETPLACES[connection.region].find((m) => m.id === mp)
              return (
                <span
                  key={mp}
            ...
            {connection.enabledMarketplaces.length === 0 && (
    ...
    <div className="space-y-6">
      ...
      {connections.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-slate-200 p-12 text-center dark:border-slate-700">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            Connect your Seller Central account to start syncing
          </p>
          <button
            onClick={onConnect}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-500 text-white font-medium text-sm hover:bg-amber-600 transition-colors"
      ...
      <div className="rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 p-4">
        ...
        <p className="text-sm text-blue-700 dark:text-blue-400">
          ...
          orders, and product data. One account can sell products from multiple internal brands.
          Each regional account (NA, EU, FE) requires a separate connection.
        </p>
      </div>
    </div>
  )
