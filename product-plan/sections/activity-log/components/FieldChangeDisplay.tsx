import type { FieldChangeDisplayProps, ChangeValue, ValueType } from '@/../product/sections/activity-log/types'
...
function getStatusColor(status: string): string {
  ...
  if (statusLower.includes('pass') || statusLower.includes('complete') || statusLower.includes('active') || statusLower.includes('delivered')) {
    return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'
  }
  if (statusLower.includes('fail') || statusLower.includes('reject') || statusLower.includes('cancel')) {
    return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
  }
  if (statusLower.includes('pending') || statusLower.includes('wait') || statusLower.includes('hold')) {
    return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'
  }
  if (statusLower.includes('transit') || statusLower.includes('progress') || statusLower.includes('ship')) {
