...
import type { POTimelineProps, ShippingMethod, StockoutGapMetric, ReasoningItem, ShippingRouteExpanded } from '../types'
...
function calculateStockoutGap(
  ...
  // Find warehouse to Amazon route if available
  const amazonTransitDays = routes?.find(r =>
    r.destinationLocationName === suggestion.destinationLocationName
  )?.totalTransitDays.typical || 3
...
export function POTimeline({ suggestion, routes }: POTimelineProps) {
  ...
  const getStatusColor = () => {
    ...
    if (gap.gapDays > -7) return 'warning'
    return 'healthy'
  }
          ...
          <div className="flex-1 mx-4 relative">
            ...
            <div className="absolute inset-x-0 -top-3 flex justify-center">
              <span className="bg-white dark:bg-slate-800 px-2 text-sm font-medium text-slate-600 dark:text-slate-400">
                {gap.totalLeadTimeDays} days via {methodLabels[(suggestion.routeMethod || 'sea') as ShippingMethod]}
              </span>
            </div>
